...
namespace Mozi.Live
{
    ...
    public class RTPServer
    {
        ...
        /// 数据包接收事件，字节流数据包
        /// </summary>
        public RTPPackageReceive DatagramReceived;
        /// <summary>
        /// 服务端口
        /// </summary>
        ...
        /// </remarks>
        public virtual ushort SendMessage(string host, int port, RTPPackage pack)
        {
            byte[] buffer = pack.GetBuffer();
            _totalSendBytes += (ulong)buffer.Length;
            _packetSendCount++;
            _socket.SendTo(buffer, host, port);
            return pack.Header.SequenceNumber;
        }
    }
    ...
    public delegate void RTPPackageReceive(string host, int port, byte[] data);
    /// <summary>
    /// 消息回调
    /// </summary>
    /// <param name="host">主机地址</param>
    ...
    public delegate void RTPMessageTransmit(string host, int port, RTPPackage rp);
}
