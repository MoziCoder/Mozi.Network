...
[新增]
增加HttpClient命令行调试工具
继续实现RTSP协议，分解RTSP所有的协议报文

[优化]
增加HttpServer,HttpClient数据统计字段
增加HttpServer,HttpClient事件
修正HttpRequest->GetRequestLine的BUG
改HttpRequest->SetHeader为AddHeader
增加HttpClient中Content-Encoding的实现，并加入两种压缩算法
增加HttpClient的多路径请求方法
抽取HttpServer->HandleError,HandleNotFound方法
去掉Common/Log.cs中的记录文件名的日期要素

[2022/06/06]
[变更]
为每个DLL生成XML注释文档，同时大量补充注释
移除HttpRequest中的ProtocolType
...
[2022/05/28]
[新增]
大幅度修改SSDPService的实现，使其更符合实用
[优化]
优化SOAP实现
为Http序列化接口加上ContentType
解决非静态文件ContentType缺失的问题
